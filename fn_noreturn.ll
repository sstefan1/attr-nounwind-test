; RUN: opt -functionattrs -disable-nounwind-inference -attributor -attributor-disable=false -S < %s | FileCheck %s
;
...
; CHECK: define dso_local void @srec0()
;
define dso_local void @srec0() #0 {
entry:
  call void @srec0()
  ret void
}
...
; FIXME: no-return missing
; CHECK: Function Attrs: noinline nounwind readnone uwtable
; CHECK: define dso_local i32 @srec16(i32 %a)
;
define dso_local i32 @srec16(i32 %a) #0 {
entry:
...
; CHECK: define dso_local i32 @endless_loop(i32 %a)
;
define dso_local i32 @endless_loop(i32 %a) #0 {
entry:
  br label %while.body
...
; FIXME: no-return missing
; CHECK: Function Attrs: noinline norecurse nounwind readnone uwtable
; CHECK: define dso_local i32 @dead_return(i32 returned %a)
;
define dso_local i32 @dead_return(i32 %a) #0 {
entry:
  br label %while.body
...
; FIXME: no-return missing
; CHECK: Function Attrs: noinline nounwind readnone uwtable
; CHECK: define dso_local i32 @multiple_noreturn_calls(i32 %a)
;
define dso_local i32 @multiple_noreturn_calls(i32 %a) #0 {
entry:
  %cmp = icmp eq i32 %a, 0
